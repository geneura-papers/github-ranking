...
\author{JJ Merelo$^{1,2,3}$\thanks{Corresponding author. He can be
    reached at {\tt jmerelo@ugr.es} or at the
    \href{https://github.com/geneura-papers/github-ranking/issues}{issues
      section} of the repo for this paper}, Israel Blancas$^1$, M. G. Arenas$^{1,3}$,
  Fernando Tricas $^4$, José Antonio Vacas\\
